        ...
        <>
            <section className="z-index-0 position-rel">
                <ul className="display-grid list-post-wrap">
                    {data.sort((a, b) => a?.createdAt?.localeCompare(b.createdAt, 'en', {sensitivity: 'base'}))?.map((event, index) => (
                        <li 
                            key={index}
                                        ...
                                        <div className="position-rel display-block box-sizing line-height-2">
                                            <Link href="/event/[slug]" as={`/event/${event.slug}`}>
                                                <a className="font-weight-3 text-black-var-1">
                                                    <div className='post-title font-inherit position-rel'>{event.title}</div>
                                                </a>
                                            </Link>
                                            <p className="mask-text-line3 m-t-8 post-content">{event.description}</p>
                                            <div className="mask-text-line3 m-t-8 post-content font-size-5">{event.location}</div>
                                            <div className="display-flex flex-algn-center post-date neutral-color-2">
                                                <div className="display-flex m-t-4 font-size-5">{getCurrentDate(event.dateFrom)} - {getCurrentDate(event.dateTo)}</div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
