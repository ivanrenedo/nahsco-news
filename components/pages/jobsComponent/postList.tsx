        ...
        <>
            <section className="z-index-0 position-rel">
                <ul className="display-grid list-post-wrap">
                    {data.sort((a, b) => a?.createdAt?.localeCompare(b.createdAt, 'en', {sensitivity: 'base'}))?.map((job, index) => (
                        <li 
                            key={index}
                                        ...
                                        <div className="position-rel display-block box-sizing line-height-2">
                                            <Link href="/job/[slug]" as={`/job/${job.slug}`}>
                                                <a className="font-weight-3 text-black-var-1">
                                                    <div className='post-title font-inherit position-rel'>{job.companyName}</div>
                                                </a>
                                            </Link>
                                            <p className="mask-text-line3 m-t-8 post-content">{job.title}</p>
                                            <p className="mask-text-line3 post-content font-size-5">{job.location}</p>
                                            <div className="display-flex flex-algn-center post-date neutral-color-2">
                                                <div className="display-flex font-size-5">{getCurrentDate(job.createdAt)}</div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
